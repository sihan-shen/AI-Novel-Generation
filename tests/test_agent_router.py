...
def test_agent_tasks_endpoint(client, db_session):
    ...
    db_session.commit()
    response = client.get("/api/project/p1/agent/tasks")
...
def test_agent_chat_stream_starts(client, db_session):
    ...
    db_session.commit()
    with client.stream("POST", "/api/project/p1/agent/chat/stream", json={"message": "Write chapter 1", "chapter_outline_id": "o1", "target_words": 1000}) as response:
        assert response.status_code == 200
        content = ""
...
def test_confirm_action_returns_error_when_no_active_task(client, db_session):
    ...
    db_session.commit()
    response = client.post("/api/project/p1/agent/chat/confirm", json={"confirm_id": "c1", "action": "approve"})
...
def test_pending_actions_when_no_active_task(client, db_session):
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["data"]["has_pending"] is False
...
def test_resume_from_with_no_active_task(client, db_session):
    ...
    db_session.add(Project(id="p1", title="Test"))
    db_session.commit()
    with client.stream("POST", "/api/project/p1/agent/chat/stream?resume_from=5", json={"message": "test", "chapter_outline_id": "x", "target_words": 10}) as response:
        assert response.status_code == 200
        content = ""
        for chunk in response.iter_text():
            content += chunk
