...
class StyleService:
    @staticmethod
    def create(db: Session, name: str, source: str = "", source_text: str = "", analysis: str = "{}", tags: str = "[]") -> Style:  # noqa: E501
        style = Style(name=name, source=source, source_text=source_text, analysis=analysis, tags=tags)  # noqa: E501
        db.add(style)
        db.commit()
        db.refresh(style)
        return style
    ...
    def set_project_styles(db: Session, project_id: str, style_weights: list[dict]) -> None:
        """Set style weights for a project. style_weights = [{"style_id": "...", "weight": 0.7}, ...]"""  # noqa: E501
        ...
        for sw in style_weights:
            link = ProjectStyleLink(project_id=project_id, style_id=sw["style_id"], weight=sw.get("weight", 1.0))  # noqa: E501
            db.add(link)
        db.commit()

    @staticmethod
    async def analyze_text(db: Session, text: str, project_id: str | None = None) -> str:
        """Send text to LLM for style analysis."""
        adapter = get_adapter(db)
        messages = [
            {"role": "system", "content": load_prompt("style_analysis")},
            {"role": "user", "content": f"请分析以下文本的文风特征：\n\n{text}\n\n输出JSON格式：{{\"vocabulary\":\"\",\"rhythm\":\"\",\"rhetoric\":\"\",\"tone\":\"\",\"dialogue\":\"\",\"pov\":\"\"}}"}  # noqa: E501
        ]
        ...
        try:
            record_usage(  # type: ignore[attr-defined]
                ...
                scenario="style_analyze", project_id=project_id,
            )
        except Exception as e:
            logger.warning("record_usage failed for style_analyze: %s", e)
        return response.content

    @staticmethod
    async def smart_slice(db: Session, text: str, project_id: str | None = None) -> list[dict]:
        """Use LLM to identify representative style slices from long text."""
        adapter = get_adapter(db)
        messages = [
            {"role": "system", "content": load_prompt("style_slice")},
            {"role": "user", "content": f'请从以下文本中选出3-5个最能代表作者文风的段落（每段200-500字），覆盖不同类型的描写（叙事、环境、心理等）。按重要度排序，每个切片附推荐理由。\n\n文本：\n\n{text[:8000]}\n\n输出JSON格式：{{"slices":[{{"index":1,"text":"...","type":"...","reason":"...","stars":5}}]}}'}  # noqa: E501
        ]
        response = await adapter.generate(messages, temperature=0.3)
        try:
            record_usage(  # type: ignore[attr-defined]
                db, adapter.model, response.usage,  # type: ignore[attr-defined]
                scenario="style_slice", project_id=project_id,
            )
        except Exception as e:
            logger.warning("record_usage failed for style_slice: %s", e)
        try:
