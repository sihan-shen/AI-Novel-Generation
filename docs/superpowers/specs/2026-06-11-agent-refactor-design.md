...
1. 敏感模式过滤：扫描所有用户数据，将包含 `<|SYSTEM|>`、`<|END|>`、`[INST]`、`</SYSTEM>` 等已知分隔标记的行替换为转义后的文本（如 `[用户数据：SYSTEM]` 替换为 `[用户数据﹕SYSTEM]`，使用不可见区别的 Unicode 字符）
...
**SSE 断连恢复**：
- 每次 `agent_start` 事件携带 `&sequence=N` 递增序号
- 前端断连重连时发送 `GET /chat/stream?task_id={id}&resume_from=N`，服务端从该序号起重放未送达的事件
...
- **保存时机**：每个 Agent 运行**完成后**保存一次快照（非每 step 保存，避免高频写入）
- **原子性保证**：快照写入采用单次数据库事务，同时更新 `blackboard_snapshot` + `orchestrator_state` + `status` 三个字段
  ```sql
  UPDATE agent_tasks
...
- **脏数据防护**：由于快照是 Agent 运行完成后才写入，不存在"快照写到一半崩溃"的场景。极端情况是快照写数据库的事务本身失败——此时 DB 回滚，下次启动时状态机回退到上一个快照点。因为上一个快照点是上一个 Agent 完成后的状态，而当前 Agent 的产出已经在 Tool handler 的独立事务中写入了数据库——所以回退安全：数据在，状态回退一个刻点，重跑即可（Tool 幂等性保证重跑安全）
...
- 因此**所有写入型 Tool 的 handler 必须是幂等的**：`write_chapter` 使用 `outline_id` 作为去重键覆盖而非新增行；`propose_setting` 使用 `key` 字段做 upsert；`submit_review` 使用 `chapter_id + triggered_by_task_id` 做 upsert
...
**草稿生命周期**：
- `write_chapter` 在写入模式为 `draft` 时：新建或**覆盖**同一 `outline_id` 对应的已有草稿（一章节一草稿，防止累积）。被覆盖的旧草稿内容写入 `agent_messages` 的一条 `metadata={old_content: ...}` 记录供对比
...
| 单次任务最大 Agent 启动次数 | `max_agent_runs` | 20 | 强制完成，汇报已产出内容 |
...
**SQLite CHECK 约束迁移提示**：SQLite 不支持 `ALTER TABLE ... ALTER COLUMN ... ADD CONSTRAINT`。如果现有 `chapters` 表已在生产数据中，增加 `status` 的 CHECK 约束需要走以下路径之一：
- **路径 A（推荐）**：在 Python 模型层用 `@validates` 或 Pydantic schema 做应用层校验，不在 SQLite 层加 CHECK 约束
- **路径 B**：创建新表 `chapters_new`（含完整 CHECK 约束）→ 迁移数据 → `DROP TABLE chapters` → `RENAME chapters_new TO chapters`

实施计划中应在 Phase 1 确定选择哪条路径。
...
| 级别 | 行为 |
|------|------|
| 每决策确认 (`all`) | 所有 `confirm_before=True` 的 Tool 都暂停请求确认 |
| 仅矛盾时暂停 (`conflict_only`) | 仅在发现设定冲突或审阅分数 < 2.5 时暂停 |
| 从不暂停 (`never`) | Agent 自行完成全部，仅在完成时汇报 |
